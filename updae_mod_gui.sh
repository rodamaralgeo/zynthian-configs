...
for d in `ls -d $ZYNTHIAN_PLUGINS_DIR/lv2/*`; do
	...
	ls -d $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins-fixed/"${B}"/modgui >/dev/null 2>&1
	if [ $? = 0 ]; then
			rsync -avP $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins-fixed/"${B}"/ /usr/lib/lv2/"${B}"
			continue
	fi
	ls -d $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins/"${B}"/modgui >/dev/null 2>&1
	if [ $? = 0 ]; then
			rsync -avP $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins/"${B}"/ /usr/lib/lv2/"${B}"
			continue
	fi
	ls -d "${d}"/modgui >/dev/null 2>&1
	if [ $? != 0 ]; then
...
for d in `ls -d $ZYNTHIAN_PLUGINS_DIR/lv2/*`; do
	...
	ls -d $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins-fixed/"${B}"/modgui >/dev/null 2>&1
	if [ $? = 0 ]; then
			rsync -avP $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins-fixed/"${B}"/ /usr/local/lib/lv2/"${B}"
			continue
	fi
	ls -d $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins/"${B}"/modgui >/dev/null 2>&1
	if [ $? = 0 ]; then
			rsync -avP $ZYNTHIAN_PLUGINS_SRC_DIR/mod-lv2-data/plugins/"${B}"/ /usr/local/lib/lv2/"${B}"
			continue
	fi
	ls -d "${d}"/modgui >/dev/null 2>&1
	if [ $? != 0 ]; then
